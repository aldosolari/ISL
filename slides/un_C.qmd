...
cat(sprintf("MSE training: %.4f\n", mean(resid(m1)^2)))
cat(sprintf("R^2 training: %.4f\n\n", summary(m1)$r.squared))
```
...
Check model assumptions (residual plot) on the fitting scale:
...
Compare the goodness-of-fit (training MSE and $R^2$) after converting predictions back to the original Y-scale:

$$
\mathrm{MSE_{trasformed}} = \frac{1}{n}\sum_{i=1}^{n} (y_i^{1/3} - \widehat{y^{1/3}_i} )^2,\qquad \mathrm{MSE_{original}} = \frac{1}{n}\sum_{i=1}^{n} (y_i - (\widehat{y^{1/3}_i})^3 )^2
$$
and 
$$
\mathrm{R^2_{trasformed}} = 1-\frac{\sum_{i=1}^{n} ( y_i^{1/3} - \widehat{y^{1/3}_i} )^2 }{ \sum_{i=1}^{n} (y_i^{1/3} - \overline{y^{1/3}} )^2}, \qquad \mathrm{R^2_{original}} = 1-\frac{\sum_{i=1}^{n} ( y_i - (\widehat{y^{1/3}_i})^3 )^2 }{ \sum_{i=1}^{n} (y_i - \bar y )^2}
$$
...
# Back-transformed predictions on original scale
y_hat_B <- fitted(m2)^3
...
MSE_B <- mean((y - y_hat_B)^2)

# R^2 on original scale
R2_B <- 1 - sum((y - y_hat_B)^2) / sum((y - mean(y))^2)

cat(sprintf("MSE original: %.4f\n", mean(resid(m2)^2)))
cat(sprintf("R^2 trasformed: %.4f\n\n", summary(m2)$r.squared))

cat(sprintf("MSE original: %.4f\n", MSE_B))
cat(sprintf("R^2 original: %.4f\n\n", R2_B))
```
...
- For deciding on the important variables, the most direct approach is called [best
